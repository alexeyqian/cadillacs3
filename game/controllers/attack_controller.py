...
class AttackController:
    ...
    def update(self, dt):
        if self.phase == AttackPhase.FINISHED:
            return
        self.phase_timer += dt
        if self.phase == AttackPhase.WINDUP and self.phase_timer >= self.current_attack.windup_dur:
            self._enter_phase(AttackPhase.ACTIVE)
        elif self.phase == AttackPhase.ACTIVE and self.phase_timer >= self.current_attack.active_dur:
            self._enter_phase(AttackPhase.RECOVERY)
        elif self.phase == AttackPhase.RECOVERY and self.phase_timer >= self.current_attack.recovery_dur:
