...
class GrabController:
    ...
    def try_grab(self, targets: List[Character]):
        ...
        for target in targets:
            ...
            if dist < 50:
                ...
                target.get_component(CharacterController).set_state("stunned")
    ...
    def throw_target(self):
        if self.grabbed_target:
            ...
            self.grabbed_target.get_component(CharacterController).set_state("idle")
            self.grabbed_target = None

            self.owner.get_component(CharacterController).set_state("idle")
